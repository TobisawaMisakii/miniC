...
- 需要在`AST.h`描述AST节点结构，包含 AST 节点的创建、管理和清理方法，以及相应的枚举类型和节点类

- 在`AttrType.h`中实现新的数据结构，比如float



### 3.SysYCSTVisitor类

在`Antlr4CSTVisitor.h`中定义了**SysYCSTVisitor**类，继承了**SysYBaseVisitor**类中所有虚函数，所有函数需要在`Antlr4CSTVisitor.cpp`中实现。
